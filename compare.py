...
class CompareWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(header_area, text=self.L("conf_titolo"), background=T.INK,
                 foreground=T.FG, font=self.theme.f_titolo).pack(anchor="w")
        tk.Label(header_area, text=self.L("conf_sotto"), background=T.INK,
                 foreground=T.MUT, font=self.theme.f_sotto).pack(anchor="w")

        choice, s = T.card(root, self.L("conf_sez_file"), self.theme)
        choice.grid(row=1, column=0, sticky="ew")
        s.columnconfigure(1, weight=1)
        for line, (key, variable) in enumerate(((self.L("conf_a"), self.var_a),
            ...
            ttk.Button(s, text=self.L("sfoglia"), width=3,
                       ...
                       command=lambda v=variable: self._scegli(v)).grid(
        ...
        ttk.Button(actions, text=self.L("conf_esegui"), style="Primario.TButton",
                   command=self.compare_images).pack(side="left")
        tk.Label(actions, text=T.micro(self.L("conf_nome")), background=T.PANEL,
                 foreground=T.MUT, font=self.theme.f_micro).pack(side="left",
                                                                padx=(16, 6))
        ttk.Entry(actions, textvariable=self.var_nome, width=16,
                  font=self.theme.f_text).pack(side="left")
        self.b_layout = ttk.Button(actions, text=self.L("conf_salva_layout"),
                                   style="Secondario.TButton",
        ...
        risultati, r = T.card(root, self.L("conf_sez_esito"), self.theme)
    ...
    def compare_images(self):
        ...
        try:
            dati_a, dati_b = A.read(a), A.read(b)
            if len(dati_a) != len(dati_b):
                return self._dillo(self.L("conf_dimensioni",
                                          a=A.human_size(len(dati_a)),
                                          b=A.human_size(len(dati_b))), T.CRIT)
            self.total_size = len(dati_a)
            self.result = A.compare_images(dati_a, dati_b)
            ...
            self._riempi(A.signatures(dati_b))
        finally:
            self.configure(cursor="")

    def _dillo(self, text, colour=T.MUT):
        self.esito_testo.configure(text=text, foreground=colour)

    def _riempi(self, signature_map):
        ...
        if not aligned:
            ...
            key = "conf_risultato_uno" if len(aligned) == 1                 else "conf_risultato"
            self._dillo(self.L(key, spans=len(aligned),
        ...
        for index, (start, end) in enumerate(aligned):
            veri = exact[index] if index < len(exact) else (start, end)
            self.table.insert("end", "0x%06X-0x%06X  0x%06X-0x%06X  %10s  " % (
                start, end, veri[0], veri[1], A.human_size(end - start + 1)))
            self.table.insert(
    ...
    def save_layout(self):
        ...
        self._dillo(self.L("conf_salvato", path=path), T.OK)
...
def open_window(parent, tm, L, folder=None):
    ...
    parent._finestra_confronto = window
    return window
